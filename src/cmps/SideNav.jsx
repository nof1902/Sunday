...
import { NavLink, Outlet, UNSAFE_DataRouterStateContext, useParams } from "react-router-dom";
...
export function SideNav({ boards, onRemoveBoard, onAddBoard, onUpdateBoard }) {
  ...
  function onBoardChange (boardid, boardTitle) {
    console.log ("board title ",boardTitle )
    onUpdateBoard ()
  }

  function onAddPressed () {
    // setIsMenuOpe n (!isMenuOpen)
    // onAddBoard
    console.log ("Add")
  }
            ...
            {filteredBoards.map((board) => (
              <div key={board._id} className="sidenav-board">
                <svg viewBox="0 0 20 20" fill="currentColor" width="19" height="19" aria-hidden="true" aria-label="Public board" class="icon_component">
                  ...
                  {/* <h2>This should edit - {editableBoardId}</h2>                */}
                 
                {editableBoardId === board._id ? (
                  <input type="text" defaultValue={board.title} onBlur={(e) => onBoardChange (e,board._id) }/>
                ) : ( 
                ...
                <button className="more-btn-show" onClick={onOpenMenu (board._id) } >
